...
const NATIVE_COLOR_PROVIDER_LANGUAGES = new Set(['css', 'scss', 'less', 'html', 'vue']);
...
suite('Default language integration coverage', () => {
	...
	DEFAULT_LANGUAGES.forEach(language => {
		test(`renders color data for ${language}`, async function () {
			...
			assert.ok(value.includes(t(LocalizedStrings.TOOLTIP_COLOR_VALUE)), `hover should include color value metadata for ${language}`);
